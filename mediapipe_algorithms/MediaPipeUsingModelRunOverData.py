...
def process_videos_in_directory(input_directory, output_csv_path):
    ...
    with open(output_csv_path, mode='w', newline='') as csv_file:
        fieldnames = ['ClipID', 'Blinking', 'Smiling', 'Head Movement']
        ...
        for video_file in tqdm(video_files, desc='Processing Videos', unit='video', ncols=80, ascii=True, file=sys.stdout):
            try:
                ...
                if counters is not None:
                    ...
                    writer.writerow({
                        'ClipID': os.path.basename(video_file),
        ...
        if face_results.multi_face_landmarks:
            ...
            if frame_count == BASELINE_FRAMES:
                ...
                if len(baseline_smile_ratios) > 0:
                    SMILE_THRESHOLD = sum(baseline_smile_ratios) / len(baseline_smile_ratios) * 1.5    # Adjust as needed
                else:
